...
import { CorzClient, type GlobalEvent } from "@corz-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<CorzClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<CorzClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<CorzClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CorzClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CorzClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://corz.test"),
...
function sdk(
  input: {
    ...
    subscribe?: CorzClient["event"]["subscribe"]
    globalEvent?: CorzClient["global"]["event"]
    promptAsync?: CorzClient["session"]["promptAsync"]
    status?: CorzClient["session"]["status"]
    messages?: CorzClient["session"]["messages"]
    children?: CorzClient["session"]["children"]
    permissions?: CorzClient["permission"]["list"]
    questions?: CorzClient["question"]["list"]
  } = {},
) {
  const client = new CorzClient()

  const subscribe: CorzClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: CorzClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CorzClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CorzClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CorzClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CorzClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CorzClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CorzClient["question"]["list"] = input.questions ?? (() => ok([]))
