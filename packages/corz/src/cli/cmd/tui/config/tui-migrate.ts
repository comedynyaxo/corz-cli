...
const TUI_SCHEMA_URL = "https://corz.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from corz.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const corz = await corzFiles(input)
  for (const file of corz) {
...
async function corzFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "corz"),
    ...(await Filesystem.findUp(["corz.json", "corz.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "corz"))
  }
  if (Flag.CORZ_CONFIG) files.push(Flag.CORZ_CONFIG)
