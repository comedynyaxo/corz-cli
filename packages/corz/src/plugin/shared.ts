...
export const DEPRECATED_PLUGIN_PACKAGES = ["corz-openai-codex-auth", "corz-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, corzVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(corzVersion) || semver.major(corzVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.corz
  if (typeof range !== "string") return
  if (!semver.satisfies(corzVersion, range)) {
    throw new Error(`Plugin requires corz ${range} but running ${corzVersion}`)
  }
}
