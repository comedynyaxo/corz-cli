...
const APP_IDS: Record<string, string> = {
  dev: "ai.corz.desktop.dev",
  beta: "ai.corz.desktop.beta",
  prod: "ai.corz.desktop",
}
const TEST_ONBOARDING = process.env.CORZ_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CORZ_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.corz.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `corz-onboarding-${randomUUID()}`)
    rmSync(root, { recursive: true, force: true })
    ;["data", "config", "cache", "state", "desktop", "session"].forEach((dir) =>
      mkdirSync(join(root, dir), { recursive: true }),
    )
    process.env.CORZ_DB = ":memory:"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Corz Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("corz://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("corz")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()

  const needsMigration = ((): boolean => {
    if (process.env.CORZ_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "corz", "corz.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.CORZ_PORT
    if (fromEnv) {
