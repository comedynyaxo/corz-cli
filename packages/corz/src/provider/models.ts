...
import { Flock } from "@corz-ai/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@corz/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.CORZ_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.CORZ_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.CORZ_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent corz CLIs can race on this cache file.
    ...
    if (!Flag.CORZ_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
