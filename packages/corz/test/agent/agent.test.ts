...
it.instance("plan agent denies edits except .corz/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".corz/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".corz", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CORZ_TEST_HOME
      process.env.CORZ_TEST_HOME = test.directory
