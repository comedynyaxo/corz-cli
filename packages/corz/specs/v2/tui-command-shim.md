...
Delete `packages/corz/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/corz/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/corz/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/corz`
- TUI plugin loader tests in `packages/corz` if runtime plugin API wiring changed
