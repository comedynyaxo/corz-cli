...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("corz-find")
  api.delete("corz-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("corz-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("corz-find", new Highlight(...rest))
    return true
  }
