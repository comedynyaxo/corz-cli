...
import "./styles.css"
import { useTheme } from "@corz-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CORZ_CHANNEL === "prod" &&
...
const deepLinkEvent = "corz:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CORZ__ ??= {}
  const pending = window.__CORZ__.deepLinks ?? []
  window.__CORZ__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("corz.global.dat").getItem("language")
