...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for corz.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal corz sessions
...
```bash
CORZ_ENABLE_QUESTION_TOOL=1 corz acp
```
...
- **Terminal Integration**: Full terminal support via corz's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | corz acp
```
...
ACP sessions map cleanly to corz's internal session model:
