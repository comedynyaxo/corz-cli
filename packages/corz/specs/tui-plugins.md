...
- Author package entrypoint is `@corz-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/corz-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@corz-ai/plugin/tui`.
- `@corz-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@corz-ai/plugin/tui"
...
- The value is a semver range checked against the running Corz version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.corz` is absent, no check is performed (backward compatible).
...
- `corz plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `corz plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.corz` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.corz`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.corz`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.corz` dir or `CORZ_CONFIG_DIR`, Corz installs `@corz-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@corz-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Corz's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@corz-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"corz"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `corz.default`.
...
- Local plugins persist installed themes under the local `.corz/themes` area near the plugin config source.
...
- `--pure` / `CORZ_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.corz/plugins/tui-smoke.tsx`
- Local vim plugin: `.corz/plugins/tui-vim.tsx`
- Local smoke config: `.corz/tui.json`
- Local smoke theme: `.corz/plugins/smoke-theme.json`
