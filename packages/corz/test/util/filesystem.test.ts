...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "corz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "corz.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "corz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "corz.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["corz.json", "corz.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "corz.json"),
        path.join(tmp.path, "corz.jsonc"),
        path.join(project, "corz.json"),
        path.join(project, "corz.jsonc"),
      ])
    })
  })
