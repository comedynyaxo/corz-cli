...
import { createCorz, type ToolPart } from "@corz-ai/sdk"
...
console.log("🚀 Starting corz server...")
const corz = await createCorz({
  port: 0,
})
console.log("✅ Corz server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await corz.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new corz session...")
    const { client, server } = corz
    ...
    console.log("✅ Created corz session:", createResult.data.id)
  ...
  console.log("📝 Sending to corz:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Corz response:", JSON.stringify(result, null, 2))
