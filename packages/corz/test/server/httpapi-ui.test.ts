...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      CORZ_DISABLE_EMBEDDED_WEB_UI: Flag.CORZ_DISABLE_EMBEDDED_WEB_UI,
      CORZ_SERVER_PASSWORD: Flag.CORZ_SERVER_PASSWORD,
      CORZ_SERVER_USERNAME: Flag.CORZ_SERVER_USERNAME,
      envPassword: process.env.CORZ_SERVER_PASSWORD,
      envUsername: process.env.CORZ_SERVER_USERNAME,
    }

    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.CORZ_DISABLE_EMBEDDED_WEB_UI = original.CORZ_DISABLE_EMBEDDED_WEB_UI
        Flag.CORZ_SERVER_PASSWORD = original.CORZ_SERVER_PASSWORD
        Flag.CORZ_SERVER_USERNAME = original.CORZ_SERVER_USERNAME
        restoreEnv("CORZ_SERVER_PASSWORD", original.envPassword)
        restoreEnv("CORZ_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            CORZ_SERVER_PASSWORD: input?.password,
            CORZ_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the experimental backend", () =>
    Effect.gen(function* () {
      Flag.CORZ_DISABLE_EMBEDDED_WEB_UI = true
      let proxiedUrl: string | undefined

      const response = yield* uiApp({
        client: httpClient(
          new Response("<html>corz</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
      ...
      expect(yield* responseText(response)).toBe("<html>corz</html>")
      expect(proxiedUrl).toBe("https://app.corz.ai/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://app.corz.ai/assets/app.js")
  ...
  it.live("requires server password for the web UI", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({ password: "secret", username: "corz" }).request("/")
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        password: "secret",
        username: "corz",
        client: httpClient(new Response("<html>corz</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("corz:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({ password: "secret", username: "corz" }).request("/", {
        headers: { authorization: `Basic ${btoa("corz:secret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "corz" }).request("/", {
        method: "OPTIONS",
        headers: {
