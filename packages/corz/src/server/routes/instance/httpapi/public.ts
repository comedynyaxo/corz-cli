...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for corz serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CorzHttpApi)
...
export const PublicApi = CorzHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "corz",
    version: "1.0.0",
    description: "corz api",
