...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".corz")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .corz/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const corz = path.join(test.directory, ".corz")
      const tool = path.join(corz, "tool")
  ...
  it.instance("loads tools from .corz/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const corz = path.join(test.directory, ".corz")
      const tools = path.join(corz, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".corz", "tools")
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".corz", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(corz, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@corz-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(corz, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(corz, "node_modules", "cowsay")
