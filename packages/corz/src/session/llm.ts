...
export class Service extends Context.Service<Service, Interface>()("@corz/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via corz's tool system
      ...
      const corzProjectID = input.model.providerID.startsWith("corz")
      ...
      return streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("corz")
            ? {
                "x-corz-project": corzProjectID,
                "x-corz-session": input.sessionID,
                "x-corz-request": input.user.id,
                "x-corz-client": Flag.CORZ_CLIENT,
