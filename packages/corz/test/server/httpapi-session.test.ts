...
describe("session HttpApi", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-corz-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* request(pathFor(SessionPaths.get, { sessionID: session.id }), {
          headers: { "x-corz-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-corz-directory": test.directory, "content-type": "application/json" }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-corz-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "corz", "src")
