...
import { createCorzClient } from "@corz-ai/sdk"
...
const { client, server } = createCorz()
...
try {
  ...
  console.log("corz session", session.id)
  if (shareId) {
...
function createCorz() {
  ...
  const proc = spawn(`corz`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createCorzClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/corz|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/corz` or `/oc`")
  }
}
...
async function assertCorzConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to corz server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.corz.ai" : "https://corz.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.corz.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("corz-github-action")
    response = await fetch("https://api.corz.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/corz" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/corz") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to corz...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "corz-agent[bot]"`
  await $`git config --global user.email "corz-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `corz/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/corz-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[corz session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
