...
import { createCorzClient } from "@corz-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "corz.mode": input.resolveSession ? "local" : "attach",
      "corz.initial_input": !!input.initialInput,
      "corz.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "corz.directory": ctx.directory,
        "corz.resume": ctx.resume === true,
        "corz.agent.name": state.agent,
        "corz.model.provider": state.model?.providerID,
        "corz.model.id": state.model?.modelID,
        "corz.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "corz.model.provider": model.providerID,
            "corz.model.id": model.modelID,
            "corz.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "corz.model.id": state.model?.modelID,
                "corz.model.variant": state.activeVariant,
                "corz.prompt.chars": prompt.text.length,
                "corz.prompt.parts": prompt.parts.length,
                "corz.prompt.include_files": includeFiles,
                "corz.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "corz.directory": input.directory,
      "corz.initial_input": !!input.initialInput,
      "corz.demo": input.demo,
    },
    async () => {
