...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CORZ_WEBSEARCH_PROVIDER

    try {
      process.env.CORZ_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CORZ_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CORZ_WEBSEARCH_PROVIDER
      else process.env.CORZ_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for corz or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.corz, { exa: false, parallel: false })).toBe(true)
