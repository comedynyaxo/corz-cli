...
Replace runtime Bun shell template-tag usage in `packages/corz/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/corz/src` is removed except:
