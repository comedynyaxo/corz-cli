...
const MANAGED_PLIST_DOMAIN = "ai.corz.managed"

// Keys injected by macOS/MDM into the managed plist that are not Corz config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/corz"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "corz")
    default:
      return "/etc/corz"
  }
}
...
export function managedConfigDir() {
  return process.env.CORZ_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
