...
import { EffectFlock } from "@corz-ai/core/util/effect-flock"
...
import { CrossSpawnSpawner } from "@corz-ai/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.CORZ_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "corz.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "corz.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://corz.ai/config.json",
      snapshot: false,
    })
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "corz.jsonc"))
    expect(content).toContain('"$schema": "https://corz.ai/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("does not create global config when CORZ_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.CORZ_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.CORZ_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "corz.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.CORZ_CONFIG_DIR
    else process.env.CORZ_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "corz.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "corz.jsonc")
...
test("ignores legacy tui keys in corz config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://corz.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "corz.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["CORZ_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { corz: { options: { apiKey: "{env:CORZ_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["corz"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["CORZ_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CORZ_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "corz.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .corz directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const corzDir = path.join(dir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })
      const agentDir = path.join(corzDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".corz", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .corz/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const corzDir = path.join(dir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })

      const agentsDir = path.join(corzDir, "agents")
...
test("loads commands from .corz/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const corzDir = path.join(dir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })

      const commandDir = path.join(corzDir, "command")
...
test("loads commands from .corz/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const corzDir = path.join(dir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })

      const commandsDir = path.join(corzDir, "commands")
...
test("does not try to install dependencies in read-only CORZ_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.CORZ_CONFIG_DIR
  process.env.CORZ_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.CORZ_CONFIG_DIR
    else process.env.CORZ_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable CORZ_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "corz.json"),
        JSON.stringify({ $schema: "https://corz.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .corz config
      const projectDir = path.join(dir, "project")
      const corzDir = path.join(projectDir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })
      ...
      // Local .corz config with different plugins
      await Filesystem.write(
        path.join(corzDir, "corz.json"),
        JSON.stringify({
          $schema: "https://corz.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .corz config with some overlapping plugins
      await Filesystem.write(
        path.join(corzDir, "corz.json"),
        JSON.stringify({
          $schema: "https://corz.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".corz")
...
// Note: preload.ts sets CORZ_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .corz config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .corz directory config enables it
      const corzDir = path.join(dir, ".corz")
      await fs.mkdir(corzDir, { recursive: true })
      await Filesystem.write(
        path.join(corzDir, "corz.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/corz")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/corz")
    ...
    if (urlStr.includes(".well-known/corz")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/corz.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/corz")
            expect(remoteFetchedUrl).toBe("https://config.example.com/corz.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "corz.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-corz@2.4.3", file)).toBe("oh-my-corz@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-corz@2.4.3", "file:///project/.corz/plugin/oh-my-corz.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.corz/plugin/demo.ts", "file:///project/.corz/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.corz/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const corzDir = path.join(projectDir, ".corz")
        const pluginDir = path.join(corzDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CORZ_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CORZ_DISABLE_PROJECT_CONFIG"]
    process.env["CORZ_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CORZ_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CORZ_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .corz/ directories when flag is set", async () => {
    const originalEnv = process.env["CORZ_DISABLE_PROJECT_CONFIG"]
    process.env["CORZ_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .corz directory with a command
          const corzDir = path.join(dir, ".corz", "command")
          await fs.mkdir(corzDir, { recursive: true })
          await Filesystem.write(path.join(corzDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .corz should NOT be in directories list
          const hasProjectCorz = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectCorz).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CORZ_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CORZ_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CORZ_CONFIG_DIR"]
      process.env["CORZ_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CORZ_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CORZ_CONFIG_DIR"]
      } else {
        process.env["CORZ_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CORZ_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CORZ_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CORZ_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CORZ_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CORZ_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from CORZ_CONFIG_DIR, not project
...
describe("CORZ_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CORZ_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CORZ_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CORZ_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://corz.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CORZ_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CORZ_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CORZ_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CORZ_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Corz Managed",
          PayloadIdentifier: "ai.corz.managed.test",
          PayloadType: "ai.corz.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://corz.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://corz.ai/config.json")
})
