...
Practical reference for a future `packages/server` split after the corz
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/corz`.
...
Until enough shared service code lives outside `packages/corz`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/corz`

It should not import `packages/corz` services while `packages/corz`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on corz-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/corz` implementation details.
