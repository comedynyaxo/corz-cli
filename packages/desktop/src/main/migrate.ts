...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.corz.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `corz.settings.dat` is special: it maps to the `corz.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"corz.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // corz.settings.dat → the electron settings store ("corz.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "corz.global.dat").
  const storeName = filename === "corz.settings.dat" ? "corz.settings" : filename
