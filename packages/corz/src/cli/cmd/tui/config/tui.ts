...
import type { DeepMutable } from "@corz-ai/core/schema"
import type { TuiAttentionSoundName } from "@corz-ai/plugin/tui"
...
export class Service extends Context.Service<Service, Interface>()("@corz/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old corz.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.corz`
  // folders between cwd and home, and CORZ_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CORZ_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CORZ_TUI_CONFIG override, if set.
  if (Flag.CORZ_TUI_CONFIG) {
    const configFile = Flag.CORZ_TUI_CONFIG
  ...
  // 4. `.corz` directories (and CORZ_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".corz") || dir === Flag.CORZ_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".corz") && dir !== Flag.CORZ_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "corz.default",
