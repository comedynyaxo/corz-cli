...
import { CorzKeymapProvider, registerCorzKeymap, useBindings, useCorzKeymap } from "./keymap"
...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.CORZ_DISABLE_MOUSE && (_config.mouse ?? true)
...
export function tui(input: {
  ...
  return new Promise<void>(async (resolve) => {
    ...
    const offKeymap = registerCorzKeymap(keymap, renderer, input.config)
    ...
    await render(() => {
      return (
        ...
        >
          <CorzKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CORZ_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CORZ_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Corz")
      return
    }
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Corz v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.CORZ_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.CORZ_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
