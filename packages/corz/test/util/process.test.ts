...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CORZ_TEST ?? "")'), {
      env: {
        CORZ_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CORZ_TEST_SHELL"], {
      shell: true,
      env: {
        CORZ_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CORZ_TEST_SHELL=ok")
  })
