import type { Hooks, PluginInput } from "@corz-ai/plugin"
import type { Model } from "@corz-ai/sdk/v2"
...
const MAK_NAME_PREFIX = "corz-oauth"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Corz - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to Corz."
...
async function createModelAccessKey(bearer: string): Promise<ApiKeyInfo> {
  ...
  const res = await fetch(`${DO_API_BASE}/v2/gen-ai/models/api_keys`, {
    ...
    headers: {
      ...
      "User-Agent": `corz/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              ...
              instructions:
                "Sign in to DigitalOcean in your browser. Corz will create a Model Access Key named corz-oauth-* and load your Inference Routers. Re-run /connect to refresh routers later.",
