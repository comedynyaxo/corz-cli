...
import { Identifier } from "@corz-ai/core/util/identifier"
import { NonNegativeInt, withStatics } from "@corz-ai/core/schema"
import { Global } from "@corz-ai/core/global"
import { AppFileSystem } from "@corz-ai/core/filesystem"

export const OAUTH_DUMMY_KEY = "corz-oauth-dummy-key"
...
export class Service extends Context.Service<Service, Interface>()("@corz/v2/Auth") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Writable, AuthError> = Effect.fnUntraced(function* () {
      if (process.env.CORZ_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.CORZ_AUTH_CONTENT)
        } catch {}
      }
