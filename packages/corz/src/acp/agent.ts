...
import type { AssistantMessage, Event, CorzClient, SessionMessageResponse, ToolPart } from "@corz-ai/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: CorzClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `corz auth login` in the terminal",
      name: "Login with corz",
      id: "corz-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Corz stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const corzProvider = providers.find((p) => p.id === "corz")
  if (corzProvider) {
    const [best] = Provider.sort(Object.values(corzProvider.models))
    if (best) {
      return {
