...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-corz-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.CORZ_WORKSPACE_ID
      Flag.CORZ_WORKSPACE_ID = WorkspaceID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.CORZ_WORKSPACE_ID = originalWorkspaceID
        }),
      )
        ...
        Effect.promise(() =>
          ExperimentalHttpApiServer.webHandler().handler(
            new Request(`http://localhost${path}`, {
              ...init,
              headers: { "x-corz-directory": dir, "content-type": "application/json", ...init?.headers },
            }),
            handlerContext,
          ),
        )
