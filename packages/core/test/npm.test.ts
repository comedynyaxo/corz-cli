...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@corz/acme")).toBe("@corz/acme")
    expect(Npm.sanitize("@corz/acme@1.0.0")).toBe("@corz/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/corz/acme.git"
    const expected = win ? "acme@git+https_//github.com/corz/acme.git" : spec
