...
import { createCorz } from "@corz-ai/sdk"
...
`)
  ...
  const corz = await createCorz({ port: 0 })
  ...
  try {
    ...
    const session = await corz.client.session.create()
    const result = await corz.client.session
      .prompt({
