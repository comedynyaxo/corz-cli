...
//      corz server and runs interactive mode against it.
//
...
import { createCorzClient, type CorzClient, type ToolPart } from "@corz-ai/sdk/v2"
import { Agent } from "@/agent/agent"
import { Permission } from "@/permission"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

const runtimeTask = import("./run/runtime")
type ModelInput = Parameters<CorzClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run corz with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running corz server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CORZ_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CORZ_SERVER_USERNAME or 'corz')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: CorzClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: CorzClient, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !Flag.CORZ_AUTO_SHARE && !args.share) return
      ...
      async function current(sdk: CorzClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: CorzClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: CorzClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: CorzClient) {
        ...
        async function loop(client: CorzClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createCorzClient({
        baseUrl: "http://corz.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
